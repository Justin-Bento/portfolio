---
title: Battling My Skills With CSS
description: MyHumber is Humber Colleges Student Dashboard
media: 04/thumbnail.png
mediaAlt: Forcon Startup Thumbnail
date: 2021-02-14T19:05:27.220Z
---

After starting at a computer day in and day out. I realized I had mental fatigue with school and being creative, I felt burnted out. So during my break I decided to step away from my laptop and pick up and hold hobby.

![First Battle On CSSBattle](/img/work/04/thumbnail.png)

## Understand The Problem

When the break was over and the new semester started, I realized I had to develop a portfolio website for my class, and I wanted to create it from scratch. But, I stoped codeing for so long and I did not know where to start again. I got nervous and thinking I had to learn everything again. I then found a YouTube video of developers testing their knowledge with CSS using cssbattles.

I decide to do this as a seprate assignment aims to help me back into web development. As well as get me back into developing my portfolio website.

## What Did I Decide To Do?

I figured out I can battle out my CSS skills by using cssbattle.dev. to help me figure out where I am with web development code. Doing this gave me a fun and exciting way to test your current knowledge with CSS. My rules for this project were simple; Have a time-limit of 25min for each challenge. Only look at MDN documentation 3x times during a challenge. Solely rely on documentation. CSSBattle also had some suggestions to help improve your ranking on their website,

1. Omit the last semicolon
2. Omit the previous closing tags
3. Remove double quotes6
4. Shorten selectors with made up attributes
5. position: fixed beats position: absolute, mostly
6. Play with the units
7. You don't always need to add an HTML element to style!
8. Omit the 'px' unit

**Disclaimer:** CSSBattle strategies should not be used in real projects. They primarily help you in CSSBattle code-golfing. In the real world, you'll probably use a minified to make your files smaller.

Once I figured that out, I then started and headed into the battlegrounds

## The Battle & Results

### Battle 01: Simply Square

![1st Battle On CSSBattle](/img/work/04/cssbattle-01.png)

High Score: 732.41 (116 chars)

Simply Square was not challenging to create but figuring out how to cut down the number of characters was difficult even with the suggestions. 1st place managed 54 characters for this challange.

### Battle 02: Carrom

![2nd Battle On CSSBattle](/img/work/04/cssbattle-02.png)

High Score: 726.24 (121 chars)

This challenge taught me how to experiment with CSS values. You experiment with values to get an accurate position.

### Battle 03: Push Button

![3rd Battle On CSSBattle](/img/work/04/cssbattle-03.png)

High Score: 726.24 (121 chars)

This challenge taught me to examine the arranging order of the stack. z-index made this possible as this changes the stack order of an elements.

### Battle 04: Ups n Downs

![4th Battle On CSSBattle](/img/work/04/cssbattle-04.png)

High Score: 726.24 (121 chars)

This challenge taught me how to apply the nth-child selector. I applied the nth-child(odd) for the 1st and 3rd container and did not touch the middle shape.

### Battle 05: Acid Rain

![5th Battle On CSSBattle](/img/work/04/cssbattle-05.png)

High Score: 726.24 (121 chars)

This challenge taught me how to adjust the border-radius with one line and helped me center and align the whole pie to the center. At this stage I am seeing improvement in my CSS and web development.

### Battle 06: Missing Slice

![6th Battle On CSSBattle](/img/work/04/cssbattle-06.png)

High Score: 726.24 (121 chars)

This challenge taught me how to adjust the border-radius with one line and helped me center and align the whole pie to the center. At this stage I am seeing improvement in my CSS and web development.

### Battle 07: Leafy Trail

![7th Battle On CSSBattle](/img/work/04/cssbattle-07.png)

High Score: 726.24 (121 chars)

Helped me understand how to use transform translation in CSS. I learned to rotate the square and change the shape.

### Battle 08: Forking Crazy

![8th Battle On CSSBattle](/img/work/04/cssbattle-08.png)

High Score: 726.24 (121 chars)

The hardest part of this challenge is figuring out how to apply the fork’s negative space to use minimal shapes. I went over the time limit.

### Battle 09: Tesseract

![9th Battle On CSSBattle](/img/work/04/cssbattle-09.png)

High Score: 726.24 (121 chars)

This challenge taught me new CSS elements clip-path the round(). The clip-path saved many characters.

### Battle 10

![10th Battle On CSSBattle](/img/work/04/cssbattle-10.png)

High Score: 548.36 (888 chars)

This challenge taught me new CSS elements clip-path the round(). The clip-path saved many characters.

## Should You Use CSSBattle?

CSSBattle is a great tool for people with experience. As you challenge your knowledge. with CSS. Testing yourself may help you get back into web development as it saved me. it helped me to get back into developmet and not dread reading documentation. This challenge also taught me new tips and tricks about CSS that I never knew existed.

However, I would not recommend it to someone starting out. There are weird convections may not be great for learning CSS. I would advocate for you to use freecodecamp. Thank you for reading my blog post and stay safe.
